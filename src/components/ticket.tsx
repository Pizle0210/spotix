...
export default function Ticket({ ticketId }: { ticketId: Id<"tickets"> }) {
  ...
  return (
    <div
      className={`overflow-hidden flex flex-col rounded-xl border bg-white shadow-xl ${ticket.event.is_cancelled ? "border-red-200" : "border-gray-100"}`}
    >
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col items-center justify-center  sm:border-l border-gray-200 pl-6">
            <div
              className={`rounded-lg bg-gray-100 p-4 ${ticket.event.is_cancelled ? "opacity-50" : ""}`}
            >
              <QRCode value={ticket._id} className='sm:w-40 sm:h-40 aspect-auto' />
            </div>
